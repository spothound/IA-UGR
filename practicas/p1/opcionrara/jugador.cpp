...
void ComportamientoJugador::NoPisar(){
    if(situado){
        reciente[fil+f2][col+c2] = tiempo+10;
    }else{
        recienteAUX[fil+f2][col+c2] = tiempo+10;
    }
}
...
Action ComportamientoJugador::think(Sensores sensores){
    ...
    if(sensores.terreno[0] == 'K' and !situado){
        ...
        for(int i = 0 ; i < 100 ; i++)
            for(int j = 0 ; j < 100 ; j++){
                ...
                if(reciente[i+despf][j+despc] != 0){
                    reciente[i][j] = recienteAUX[i+despf][j+despc];
                    pisado[i][j] = pisadoAUX[i+despf][j+despc];
                    memoria[i+despf][j+despc] = memoriaAUX[i+despf][j+despc];
                }
            }
    }
        ...
        for(int j = 0-(f*i) ; j != 0+(f*i)+f ; j+= f){
            ...
            if(situado){
                ...
                if(contador == 2 or contador == 3)
                    reciente[fi][co] = tiempo ;
            }
            else{
                mapaAUX[fi][co] = sensores.terreno[contador] ;
                if(contador == 2 or contador == 3)
                    recienteAUX[fi][co] = tiempo;
            }
            contador++ ;
        }
    }
    ...
    if(situado){
        pisado[fil][col] += 1;
        reciente[fil][col] = tiempo;
        memoria[fil][col] = pisado[fil][col] + reciente[fil][col];
    }else{
        pisadoAUX[fil][col] += 1;
        recienteAUX[fil][col] = tiempo;
        memoriaAUX[fil][col] = pisadoAUX[fil][col] + recienteAUX[fil][col];
        }
