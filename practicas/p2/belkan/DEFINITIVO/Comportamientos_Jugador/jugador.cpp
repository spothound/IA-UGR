...
int ComportamientoJugador::NuevoVisible(char d){
    int fill = fil, coll = col;
    int bru = brujula ;
    int contador = 1 ;
    int a,b,c,f; // a y b fila o columna e suma o resta filas.
...
bool ComportamientoJugador::breadth_first_search(Estado inicio, Estado objetivo)
{
    ...
    while(!frontera.empty() and iteraciones < 7000){
...
bool ComportamientoJugador::A_estrella_search(Estado inicio, Estado objetivo)
{
    ...
    while(!frontera.empty() and iteraciones < 600){
        ...
        */
        ...
        char tn,ta;
        if(situado){
            tn = mapaResultado[next.fila][next.columna] ;
            ta = mapaResultado[actual.fila][actual.columna];
        }else{
            tn = mapaAUX[next.fila][next.columna] ;
            ta = mapaAUX[actual.fila][actual.columna];
        }

        if(tn == 'D'){
            cercanos.insert(actual);
            cercania+=30;
        }
...
bool ComportamientoJugador::MejorGiroIzquierda(){
    int nvi = NuevoVisible('i') ;
   //cout << "\tNVI: " << nvi << flush ;
    int nvd = NuevoVisible('d') ;
   //cout << "\tNVD: " << nvd <<flush;
...
void ComportamientoJugador::Pulgarcito(){
   ...
   //cout << "\tNVA: " << nva <<flush ;
    if(MejorGiroIzquierda()){
        int nvi = NuevoVisible('i');
        if(nvi < nva or delante == 'P')
            accion = actTURN_L ;
        else if(nva == nvi and izquierda < avanza)
            accion = actTURN_L;
       //cout << "\tNVI: " << nvi <<flush;
    }else{
    int nvd = NuevoVisible('d');
    if(nvd < nva or delante == 'P')
        accion = actTURN_R;
    else if(nva == nvd and derecha < avanza)
        accion = actTURN_R;
   //cout << "\tNVD: " << nvd <<flush;
    }
}
    ...
    for(int i = 0 ; i < 100 ; i++)
        for(int j = 0 ; j < 100 ; j++){
            ...
            if(memoriaAUX[i+despf][j+despc] != 0){
                vistos[i][j] = vistosAUX[i+despf][j+despc];
...
void ComportamientoJugador::Cartografiar(Sensores sensores){
    ...
    for(int i = 1 ; i <= 3 ; i++){
        for(int j = 0-(f*i) ; j != 0+(f*i)+f ; j+= f){
            ...
            if((sensores.superficie[contador] == '1' and !lotengo[1]) or
               (sensores.superficie[contador] == '2' and !lotengo[2]) or
               (sensores.superficie[contador] == '3' and !lotengo[3])){
                   Estado newobjetivo ;
                   newobjetivo.orientacion = 0 ;
                   newobjetivo.fila = fi ;
                   newobjetivo.columna = co;
                   cercanos.insert(newobjetivo);
               }
            if(situado){
                memoria[fi][co]++ ;
                ...
                if(sensores.terreno[contador] == 'K'){
                    Estado newobjetivo ; newobjetivo.orientacion = 0;
                    newobjetivo.fila = fi ; newobjetivo.columna = co ;
                    cercanos.insert(newobjetivo) ;
                }
                vistosAUX[fi][co]++;
            }
...
Action ComportamientoJugador::think(Sensores sensores){
    ...
    if(cercanos.size() > 0 and !estoy_ejecutando_plan and cercania <= 0){
        cout << "\nVOY A COGÉ ARGO!" << flush;
        set<Estado>::iterator it = cercanos.begin() ;
        //estoy_ejecutando_plan = breadth_first_search(origen,*it);
        estoy_ejecutando_plan = A_estrella_search(origen, *it);
        if(estoy_ejecutando_plan)
    ...
    // planificación.
    if (sensores.vida > 25 and paseo == 0 and situado and !estoy_ejecutando_plan){
        ...
        if(!lotengo[4] and sensores.regalos.size()>0){
            ...
            int k = 0;
            while(!destinos.empty() and !estoy_ejecutando_plan and k < 3){
                destino = destinos.get();
                if(Distancia(origen, destino) < 20)
        ...
        if(lotengo[4] and objetivos.size() > 0){
            ...
            int j = 0 ;
            while(!destinos.empty() and !estoy_ejecutando_plan and j < 3){
                destino = destinos.get();
                if(Distancia(origen, destino) < 20)
                    estoy_ejecutando_plan = A_estrella_search(origen, destino) ;
                j++;
        ...
        if(!estoy_ejecutando_plan and accion == actFORWARD){
            ...
            if(sensores.vida < 25){
                ...
                if(accion == actFORWARD)
                    if(encimade != 'A' and encimade != 'B'){
                        if(miobj != '_')
                            if(delante == 'S' or delante == 'T')
                                if(sensores.vida%3==0)
                                    accion = actPUTDOWN;
                    }else if(supd == 'A' or supd == 'B')
                        Giro();
            }
